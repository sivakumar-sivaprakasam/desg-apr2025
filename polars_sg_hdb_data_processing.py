...
df = df.with_columns(flat_type = pl                     
...
df_joined = df_min.join(df_max, on=["town", "decade"]).select(
    ...
    # 3RM (MIN) & 3RM (MAX)
    pl.col("3RM").alias("3RM (MIN)"), 
    pl.col("3RM_right").alias("3RM (MAX)"), 

    # 4RM (MIN) & 4RM (MAX)
    pl.col("4RM").alias("4RM (MIN)"), 
    pl.col("4RM_right").alias("4RM (MAX)"), 

    # 5RM (MIN) & 5RM (MAX)
    pl.col("5RM").alias("5RM (MIN)"), 
    pl.col("5RM_right").alias("5RM (MAX)"), 

    # EXEC (MIN) & EXEC (MAX)
    pl.col("EXEC").alias("EXEC (MIN)"), 
    pl.col("EXEC_right").alias("EXEC (MAX)"), 

    # MULTI-GEN (MIN) & MULTI-GEN (MAX)
    pl.col("MULTI-GEN").alias("MULTI-GEN (MIN)"), 
    pl.col("MULTI-GEN_right").alias("MULTI-GEN (MAX)")
).sort("town", "decade")

print(df_joined)

df_joined.write_csv("output\sg_hdb_analysis_result.csv", include_header=True)
